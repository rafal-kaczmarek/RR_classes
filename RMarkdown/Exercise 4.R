...
setwd("C:\\Users\\Rafal\\Git_projects\\RR_classes\\april_19_23")
...
#load task data
task_data = read.csv("Data\\onet_tasks.csv")
#check the number of sheets in Eurostat_employment_isco.xlsx
isco_len <- length(excel_sheets("Data\\Eurostat_employment_isco.xlsx") ) - 1
#load isco sheets 
for (i in 1:isco_len) {
  assign(paste0("isco", i), read_excel("Data\\Eurostat_employment_isco.xlsx", sheet = paste0("ISCO", i)))
}
#Function to calculate worker totals in each of the chosen countries.
worker_total <- function(country){
  paste0("total_",country) = for (i in 1:isco_len){
    sum(get(paste0("isco",i,"$",country)))
  }
}

#it doesn't work but I tried many times
total_Belgium = isco1$Belgium + isco2$Belgium + isco3$Belgium + isco4$Belgium + isco5$Belgium + isco6$Belgium + isco7$Belgium + isco8$Belgium + isco9$Belgium
total_Spain = isco1$Spain + isco2$Spain + isco3$Spain + isco4$Spain + isco5$Spain + isco6$Spain + isco7$Spain + isco8$Spain + isco9$Spain
total_Poland = isco1$Poland + isco2$Poland + isco3$Poland + isco4$Poland + isco5$Poland + isco6$Poland + isco7$Poland + isco8$Poland + isco9$Poland
...
i=1
while (i <= isco_len){
  all_total_Belgium <-  c(all_total_Belgium,total_Belgium)
  all_total_Spain <- c(all_total_Spain,total_Spain)
  all_total_Poland <- c(all_total_Poland,total_Poland)
  i=i+1
}

all_data$total_Belgium <- all_total_Belgium
all_data$total_Spain <- all_total_Spain
all_data$total_Poland <- all_total_Poland
